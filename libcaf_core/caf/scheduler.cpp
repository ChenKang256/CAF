...
#include "caf/detail/priority_queue.hpp"
...
struct worker_data {
  ...
  // This queue is exposed to other workers that may attempt to steal jobs
  // from it and the central scheduling unit can push new jobs to the queue.
  detail::priority_queue<resumable> queue; // TODO:
    ...
    for (;;) {
      ...
      switch (res) {
        case resumable::resume_later: {
          ...
          // voluntarily released the CPU to let others run instead this means
          // we are going to put this job to the very end of our queue.
          // data_.queue.unsafe_append(job, job->getPrio());
          break;
        }
        case resumable::done: {
          intrusive_ptr_release(job);
...
public:
  ...
  void schedule(resumable* ptr) override {
    int8_t core = ptr->getCore();
    if (0 <= core && core < cpu_num_) {
      // 处理器核号合法, 调度到对应线程的queue上
      auto w = this->worker_by_id(core);
      w->schedule(ptr);
    } else {
      // 非法, 留在全局queue上
      printf("schedule prio %d\n", ptr->getPrio());
  ...
  void start() override {
    ...
    worker_data init{this};
    // Prepare workers vector.
    workers_.reserve(cpu_num_);
    // Create worker instances.
    for (size_t i = 0; i < cpu_num_; ++i)
      // i is id and cpu id
      workers_.emplace_back(
  ...
  void stop() override {
    ...
    // Use a set to keep track of remaining workers.
    shutdown_helper sh; // TODO: 这里优先级默认是127, 是否会有问题? 可能需要修改成0?
    std::set<worker_type*> alive_workers;
    for (size_t i = 0; i < cpu_num_; ++i) {
      alive_workers.insert(worker_by_id(i));
      sh.ref(); // Make sure reference count is high enough.
    ...
    // 清空queue
    for (auto job = queue.try_take_head(); job != nullptr; job = queue.try_take_head()) {
      detail::cleanup_and_release(job);
    }
  }

  detail::priority_queue<resumable> queue;
...
std::unique_ptr<scheduler> scheduler::make_work_priority(actor_system& sys) {
  return std::make_unique<work_priority::scheduler_impl>(sys);
}
